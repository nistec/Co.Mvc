...
namespace Netcell.Data.Db.Entities
{
    ...
    public class Trans_Batch_Context : EntityContext<Trans_Batch>
    {
        ...
        public static DataTable GetCampaignBatches(int CampaignId)
        {
            ...
            {
                return cmd.ExecuteCommand<DataTable>("select * from Trans_Batch where CampaignId=@CampaignId", DataParameter.GetSql("CampaignId", CampaignId));
            }
        }
        public static string Lookup_TransBatchComment(int BatchId)
        {
            using (IDbCmd cmd = NetcellDB.Instance.NewCmd())
            {
                return cmd.LookupQuery<string>("Comment","Trans_Batch_Sent", "BatchId=@BatchId","",new object[]{ BatchId});
            }
        }
        public static Trans_Batch Deserialize(string base64)
        {
            return BinarySerializer.DeserializeFromBase64<Trans_Batch>(base64);
        }
