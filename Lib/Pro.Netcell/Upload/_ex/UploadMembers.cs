...
namespace Pro.Lib.Upload
{
    ...
    {
        const string MappingName = "Party_Members_Upload";

        const string ProcName = "sp_Party_Upload_Members";

        const string ProcUploadCatName = "sp_Party_Upload_Members_Cat";
        ...
        //public static DataTable CsvSchema()
        //{
        //    DataTable dt = new DataTable("Csv_Members");
        ...
        //public static UploadSumarize DoUploadStg(IDbContext db, int AccountId, string fileName, bool updateExists, ContactsUploadMethod method)
        //{
        ...
        //        return con.Upload(dt, updateExists, ContactsUploadMethod.Stg);
        ...
        void FillLLookups()
        {
            ...
            using (IDbCmd cmd = Db.DbCmd())
            {
                ...
                listCity = UploadReader.ReadLookup(cmd, "Party_Cities", filter, "CityName", "CityId");
                listPlace = UploadReader.ReadLookup(cmd, "Party_PlaceOfBirth", filter, "PlaceName", "PlaceId");
                listCharge = UploadReader.ReadLookup(cmd, "Party_Charge", filter, "ChargeName", "ChargeId");
                listBranch = UploadReader.ReadLookup(cmd, "Party_Branch", filter, "BranchName", "BranchId");
                listStatus = UploadReader.ReadLookup(cmd, "Party_Enums", filter + " and PropType='Status'", "PropName", "PropId");
                //listRegion = UploadReader.ReadLookup(cmd, "Party_Enums", filter + " and PropType='Region'", "PropName", "PropId");
                listRegion = UploadReader.ReadLookup(cmd, "Party_Region", filter, "RegionName", "RegionId");
            }
        }
        ...
        public object ReadField(DataRow dr, FieldsMap fm)
        {
            try
            {
                if (!fm.IsExists)
                ...
                {
                    case "text":
                        return UploadReader.ReadValidTextField(dr, fm.FieldMap, (string)fm.DefaultValue, fm.FieldLength);
                    case "date":
                        return UploadReader.ReadValidDateField(dr, fm.FieldMap, Types.ToDateTime(fm.DefaultValue));
                    case "sdate":
                        return UploadReader.ReadValidStrDateField(dr, fm.FieldMap, fm.DefaultValue);
                    case "sex":
                        return UploadReader.ReadValidSexField(dr, fm.FieldMap, (string)fm.DefaultValue);
                    case "combo":
                        switch (fm.Src)
                        {
                            case "listBranch":
                                return UploadReader.ReadValidLookupField(dr, fm.FieldMap, (int)fm.DefaultValue, listBranch);
                            case "listCharge":
                                return UploadReader.ReadValidLookupField(dr, fm.FieldMap, (int)fm.DefaultValue, listCharge);
                            case "listCity":
                                return UploadReader.ReadValidLookupField(dr, fm.FieldMap, (int)fm.DefaultValue, listCity);
                            case "listPlace":
                                return UploadReader.ReadValidLookupField(dr, fm.FieldMap, (int)fm.DefaultValue, listPlace);
                            case "listRegion":
                                return UploadReader.ReadValidLookupField(dr, fm.FieldMap, (int)fm.DefaultValue, listRegion);
                            case "listStatus":
                                return UploadReader.ReadValidLookupField(dr, fm.FieldMap, (int)fm.DefaultValue, listStatus);
                            default:
                                return fm.DefaultValue;
                        }
                    case "cli":
                        return UploadReader.ReadValidMobileField(dr, fm.FieldMap, (string)fm.DefaultValue);
                    case "phone":
                        return UploadReader.ReadValidPhoneField(dr, fm.FieldMap, (string)fm.DefaultValue);
                    case "mail":
                        return UploadReader.ReadValidEmailField(dr, fm.FieldMap, (string)fm.DefaultValue);
                    case "bool":
                        return UploadReader.ReadValidBoolField(dr, fm.FieldMap, Types.ToBool(fm.DefaultValue,false));
                    case "int":
                    case "auto":
                    case "guid":
                    default:
                        return UploadReader.ReadValidField(dr, fm.FieldMap, fm.DefaultValue);
                }
        ...
        public object ReadFieldStg(DataRow dr, FieldMapItem fm)
        {
            try
            {
                //if (!fm.IsExists)
        ...
        public UploadSumarize Upload(DataTable dtCustomer, bool updateExists, ContactsUploadMethod method)
        {
            ...
            try
            {
                ...
                //CreateUploadTables(dtCustomer, updateExists);
                ...
                if (method == ContactsUploadMethod.Stg)
                {
                    ...
                    {
                        bulkCopy.BulkInsert(dtMembers, "Party_Upload_STG", null);
                    }
                }
                else //if (method != ContactsUploadMethod.Preload)
                {
                    using (DbBulkCopy bulkCopy = new DbBulkCopy(Db))
                    {
                        bulkCopy.BulkInsert(dtMembers, "Party_Upload_Members", null);
                        //if (dtCategories != null && dtCategories.Rows.Count > 0)
                        //{
                        //    bulkCopy.BulkInsert(dtCategories, "Party_Upload_MembersCategory", null);
                        //}
                        //bulkCopy.BulkInsert(dt_Manager, "Party_Upload_Manager", null);

                    }

                    //if (method != ContactsUploadMethod.Preload)
                    //    this.ExecUpload(method, UploadKey, count, updateExists);
                }
        ...
        internal int CreateUploadTableStg(DataTable dtFile, bool updateExists)//, ContactsUploadMethod method)
        {
            ...
            var map = FieldsMapper.CreateMapFieldsSemantic(AccountId, dtFile);
        ...
        internal int CreateUploadTableStg_old(DataTable dtFile, bool updateExists)//, ContactsUploadMethod method)
        {
            ...
            foreach (DataRow row in dtFile.Rows)
            {
                try
                {
                    ...
                    foreach (var fm in map)
                    {
                        ...
                        object fieldVal = ReadFieldStg(row, fm);
                        if (fm.Require && fieldVal == null)
                        {
                            isValid = false;
                            break;
                        }
                        values[fm.FieldOrder] = fieldVal;
                    }
                    ...
                    map.SetPosValue<int>(values, map.PosAccountId, AccountId);
                    map.SetPosValue<int>(values, map.PosRule, (int)0);
                    map.SetPosValue<DateTime>(values, map.PosNow, Now);
                    map.SetPosValue<int>(values, map.PosRecordId, count);
                    map.SetPosValue<string>(values, map.PosUploadKey, UploadKey);
                    map.SetPosValue<int>(values, map.PosState, 0);
        ...
        internal int CreateUploadTableMap(DataTable dtFile, bool updateExists)//, ContactsUploadMethod method)
        {
            ...
            foreach (DataRow row in dtFile.Rows)
            {
                try
                {
                    ...
                    foreach(var fm in map)
                    {
                        object fieldVal = ReadField(row, fm);//fm.FieldMap, fm.FieldType, fm.FieldLength, fm.Src, fm.DefaultValue, fm.Require, fm.IsValid);
                        if(fm.Require && fieldVal==null)
                        {
                            isValid = false;
                            break;
                    ...
                    string memberId = map.GetPosValue<string>(values,map.PosMemberId,null);
                    ...
                    string cellNumber = map.GetPosValue<string>(values, map.PosCli, null);
                    string email = map.GetPosValue<string>(values, map.PosMail, null);
                    ...
                    map.SetPosValue<int>(values, map.PosAccountId, AccountId);
                    map.SetPosValue<int>(values, map.PosRule, (int)rule);
                    map.SetPosValue<DateTime>(values, map.PosNow, Now);
                    map.SetPosValue<int>(values, map.PosRecordId, count);
                    map.SetPosValue<string>(values, map.PosUploadKey, UploadKey);
                    map.SetPosValue<int>(values, map.PosState, 0);
        ...
        public static void ExecUploadMemberAsync(int accountId, int category, string uploadKey, int updateExists)
        {
            ...
            DbPro.Instance.ExecuteNonQuery("sp_Upload_Manager_Add", 
                "UploadKey",uploadKey,
                "UploadType","stg",
                "AccountId",accountId,
            ...
            //            "CommandType", "proc",
            //            "CommandText", "netcell_sb.dbo.sp_Party_Upload_Members_Cat_Stg_Async",
            //            "Arguments", uploadKey,
            //            "DbName", "Netcell_SB",
        ...
        public static MembersUploadSumarize ExecUploadMemberCatProcedure(IDbContext db, int accountId, int category, string uploadKey, int updateExists)
        {
            ...
            //    UploadKey = uploadKey,
            //    UploadState = 0,
            //    UploadType = "Party_Upload_Members"
            //});
            ...
            db.ExecuteNonQuery(ProcUploadCatName,parameters.ToArray(), CommandType.StoredProcedure);
        ...
        public static int ExecDeleteMembersByCategory(IDbContext db, int AccountId, int Category)
        {
            ...
            db.ExecuteNonQuery("sp_Party_Members_Delete", parameters.ToArray(), CommandType.StoredProcedure);
