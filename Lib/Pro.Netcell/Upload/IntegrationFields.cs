...
namespace Pro.Netcell.Upload
{
    public class IntegrationFields : IEntityItem
    {
        ...
        public static Dictionary<string, IntegrationFields> LoadFieldsMap(int SourceType)
        {
            using (var db = DbContext.Create<DbPro>())
            {
                var list = db.ExecuteList<IntegrationFields>("select * from [vw_Integration_Fields_Map] where SourceType=@SourceType", "SourceType", SourceType);
