...
namespace ProNetcell.Data.Entities
{
    ...
    [Entity(EntityName = "MemberItem", MappingName = "Members_Items", ConnectionKey = "netcell_db", EntityKey = new string[] { "RecordId,AccountId" })]
    public class MemberContext : EntityContext<MemberItem>
    {
        ...
        public static int DoSave(MemberItem v, bool UpdateExists ,string DataSource,DataSourceTypes DataSourceType)
        {
            ...
            using (var db = DbContext.Create<DbNetcell>())
            {
                int res = db.ExecuteCommandNonQuery("sp_Member_Save", parameters.ToArray(), System.Data.CommandType.StoredProcedure);
           ...
           int ContactRule = 0)
        {
            using (var db = DbContext.Create<DbNetcell>())
            {
                return db.ExecuteList<MemberListView>("sp_Members_Query", "QueryType", QueryType, "PageSize", PageSize, "PageNum", PageNum, 
                    "coAccountId", AccountId,
                    //"MemberId", MemberId,
                    "ExId", ExId,
                    "CellPhone", CellPhone,
                    "Email", Email,
        ...
        public static IEnumerable<MemberListView> ListQueryView(MemberQuery q)
        {
            using (var db = DbContext.Create<DbNetcell>())
            {
                return db.ExecuteList<MemberListView>("sp_Members_Query", "QueryType", q.QueryType, "PageSize", q.PageSize, "PageNum", q.PageNum, 
                    "coAccountId", q.AccountId,
                    //"MemberId", q.MemberId,
                    "ExId", q.ExId,
        ...
        public static DataTable ListQueryDataView(MemberQuery q)
        {
            using (var db = DbContext.Create<DbNetcell>())
            {
                return db.ExecuteCommand<DataTable>("sp_Members_Query", DataParameter.GetSql("QueryType", q.QueryType, "PageSize", q.PageSize, "PageNum", q.PageNum, 
                    "coAccountId", q.AccountId,
                    //"MemberId", q.MemberId,
                    "ExId", q.ExId,
                    "CellPhone", q.CellPhone,
        ...
        public static int ListQueryTargetsView(MemberQuery q)
        {
            using (var db = DbContext.Create<DbNetcell>())
            {
                return db.ExecuteScalar<int>("sp_Members_Query_Targets", 0, "QueryType", q.QueryType, 
                    "coAccountId", q.AccountId, "UserId", q.UserId,
                    //"MemberId", q.MemberId,
                    "ExId", q.ExId,
                    "CellPhone", q.CellPhone,
                    "Email", q.Email,
    ...
    public class MemberContext<T> : EntityContext<DbNetcell, T> where T : IEntityItem
    {
