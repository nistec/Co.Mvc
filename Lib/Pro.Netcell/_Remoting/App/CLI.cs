...
namespace Netcell.Remoting
{
    public struct CLI
    {
        ...
        public static bool ValidatePhoneCall(string link)
        {
            ...
            return Nistec.Regx.RegexValidateIgnoreCase(pattern, GetCliTel(link));
        }
        ...
        {
              return Nistec.Regx.RegexValidateIgnoreCase(MobilePattern, cli);
        }
        public static string  CleanCli(string cli)
        {
            return Nistec.Regx.RegexReplace(@"[^0-9]", cli, "");
        }
        ...
        public static string FixNumber(string cellNumber)
        {
            ...
            if (!ValidateMobile(CleanNumber))
                return string.Empty;
            return Nistec.Regx.RegexReplace(@"^(0|972|)", CleanNumber, "972");
...
/*            
            ...
            char[] chrNumber = cellNumber.ToCharArray();
            foreach(char c in chrNumber)
            {
                if (Char.IsDigit(c))
                {
                    CleanNumber += c.ToString();
                }
            }
            int length = CleanNumber.Length;
            if (CleanNumber.Substring(0, 2).Equals("00"))
            {
            ...
            switch (length)
            {
                ...
                case 10:
                        if (CleanNumber.StartsWith("05"))
                            ...
                            return cellNumber;
                case 12:
                        if (CleanNumber.StartsWith("972") && CleanNumber.Substring(3, 1) != "5")
                            return string.Empty;
                        else
                            return CleanNumber;
                default:
                    if ((CleanNumber.Length > 12) || (CleanNumber.Length < 9))
                    {
                        return string.Empty;
                    }
...
*/
        ...
        public static bool TryParse(ref string cellNumber)
        {
            ...
            if (!ValidateMobile(CleanNumber))
                return false;
            cellNumber = Nistec.Regx.RegexReplace(@"^(0|972|)", CleanNumber, "972");
            return true;
        ...
        //static public int GetDefaultOperator(int cellId)
        //{
        //    string fixNumber = "972" + cellId.ToString().Substring(0, 2);
        //    return GetDefaultOperator(fixNumber);
        //}

        #endregion
    }

    //public enum OperatorProvider
    //{
    //    NA = 0,
    //    Cellcom = 1,
    //    Orange = 2,
