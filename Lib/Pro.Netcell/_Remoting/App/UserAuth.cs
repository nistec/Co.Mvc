...
{
    public class UserAuth : Nistec.Data.Entities.ActiveEntity
    {
        ...
        public UserAuth(string LogInName, string Pass)
        {
            try
            {
                ...
                LogInName = GetValue<string>("LogInName");
                Pass = GetValue<string>("Pass");
                UserId = GetValue<int>("UserId");
                AccountId = GetValue<int>("AccountId");
                UserType = GetValue<int>("UserType");
                ExState = 0;
                if (AccountId < 0)
                {
                    throw new MsgException(AckStatus.AuthorizationException, "Un Authorized");
