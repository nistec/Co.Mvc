...
namespace Pro.Lib.Payments.Server
{
    ...
    public class PaymentAgent
    {
        ...
        public void Start()
        {
            ...
            Netlog.Debug("Start Scheduler");
            keepAlive=true;
            threadPool = new GenericThreadPool( AgentsConfig.SchedulerMaxThread);
        ...
        */
        ...
        private void SchedulerProcess(object state)
        {
            ...
            while (keepAlive)
            {
                 ...
                 try
                 {
                     ...
                     lock (m_lock)
                     {
                         ...
                         if (queueItem != null && queueItem.IsValid)
                         {
                             ...
                             {
                                 //TODO ADD ALERT
                                 Netlog.ErrorFormat("SchedulerState is Expired: " + queueItem.QueueId);
                             }
                             else
                             {
                                 Interlocked.Increment(ref m_Connections);
                 ...
                 {
                     Netlog.WarnFormat("SchedulerProcess ThreadAborted");
                 }
                 catch (Exception ex)
                 {
                      Netlog.ErrorFormat("Scheduler error:{0} ", ex.Message + " Trace:" + ex.StackTrace);
            ...
            Netlog.WarnFormat("Scheduler not keep Alive");
        }
        ...
        void SchedulerWorker(PaymentChargeItem queueItem)
        {
            try
            {
                if (queueItem != null && queueItem.IsValid)
                {
                    Netlog.DebugFormat("SchedulerWorker ExecuteAsync {0}", queueItem.QueueId);
            ...
            {
                Netlog.WarnFormat("SchedulerWorker ThreadAborted");
            }
            catch (Exception ex)
            {
                Netlog.ErrorFormat("SchedulerWorker error:{0} ", ex.Message);
            }
        ...
        public void SingleProcess()
        {
            ...
            try
            {
                ...
                lock (m_lock)
                {
                    ...
                    Netlog.Debug("PaymentChargeGet Dequeue...");
                    ...
                    if (queueItem != null && queueItem.IsValid)
                    {
                        ...
                        {
                            //TODO ADD ALERT
                            Netlog.ErrorFormat("PaymentCharge State is Expired: " + queueItem.QueueId);
                        }
                        else
                        {
                            Netlog.DebugFormat("PaymentCharge begin Payed_Id:{0}", queueItem.Payed_Id);
                            PaymentBroker.ChargeWithToken(queueItem);
        ...
        public void Stop()
        {
            ...
            {
                Netlog.ErrorFormat("Stop Scheduler error:{0}", ex.Message);
            }
        }
    }
}
