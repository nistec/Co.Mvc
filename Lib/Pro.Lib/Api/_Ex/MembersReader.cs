...
namespace Pro.Lib.Api
{
    ...
    internal class MembersReader : IDisposable
    {
        public const int DefaultMinMembersUploadAsync = 100;
        ...
        {
            MinMembersUploadAsync = WebConfig.MinMembersUploadAsync;
            dal = DalMembers.Instance;
            dal.AutoCloseConnection = false;
            errs = new List<string>();
        }
        ~MembersReader()
        ...
        internal static Dictionary<string, string> MapFields(int accountId)
        {
            ...
            var dt = MemberApiContext.MemberCategoryFields(accountId);
            ...
            //var dic = MemberContext.MemberCategoryDictionary(accountId);
            ...
            var map = MemberApiContext.MemberCategoryMap(accountId);
        ...
        public UploadSumarize MembersItemsUpload(DataTable dtUpload, int accountId, int groupId, MemberUpdateType updateType, MembersUploadMethod method, MemberKeyType keyType, Guid uploadKey)
        {
            ...
            if (keyType == MemberKeyType.None)
            {
                keyType = (MemberKeyType)this.dal.Lookup_MemberKeyType(accountId);
            }
            UploadSumarize sumarize = new UploadSumarize();
            DataTable dt = this.dal.MembersUploadSchema().Clone();
            sumarize.WrongItems = dtUpload.Clone();
            dt.Constraints.Add("IX_Members_Upload", new DataColumn[] { dt.Columns["Identifier"] }, false);
            ...
            foreach (DataRow row in dtUpload.Rows)
            {
                try
                {
                    ...
                    MemberRule rule = MemberApiContext.ValidateMemberRule(ref cellNumber, ref email);
                    identifier = MemberContext.GetIdentifier(keyType, accountId, cellNumber, email, exKey, rule);
                    if (identifier == null)
                    {
                    ...
                    values[0] = count;
                    values[1] = accountId;
                    values[2] = MemberApiContext.ValidateTarget(cellNumber);
                    values[3] = MemberApiContext.ValidateTarget(email);
                    ...
                    values[11] = (int)MemberSignType.Upload;
            ...
            {
                throw new Exception("Members list not define correctly");
            }
            if (count <= 0)
            {
                throw new Exception("No Members inserted");
            }
            try
            {
                if ((dt != null) && (dt.Rows.Count > 0))
                {
                    ...
                    //MemberUpdateType updateType = updateExists ? MemberUpdateType.UpdateFull : MemberUpdateType.InsertOnly;
                    bool updateExists = updateType != MemberUpdateType.InsertOnly;
                    if (method == MembersUploadMethod.Auto)
                    {
                        if (!updateExists || count <= MinMembersUploadAsync)
                        {
                            // go to Members_Upload_Exec
                            method = MembersUploadMethod.Sync;
                        }
                        else
                        {
                            method = MembersUploadMethod.QueueCommand;
                        }
                    }
                    this.dal.InsertTable(dt, "Members_Items_Upload");
                    dal.Members_Upload_Mng_Insert(uploadKey.ToString(), (int)updateType, groupId, accountId, (int)method);
                    if (method == MembersUploadMethod.Sync)
                    {
                        dal.MembersItems_Upload_Exec(uploadKey.ToString());
                    }
                    else
                    {
        ...
        private void ExecUploadEx(MembersUploadMethod method, string uploadKey, int count, bool updateExists)
        {
            //if (method == MembersUploadMethod.AsyncDbCmd)
            //    dal.App_AsyncCmd_Insert("sp_Members_Upload", uploadKey.ToString());

            if (method == MembersUploadMethod.QueueCommand)
            {
                RemoteApi.Instance.ExecuteQueueCommand("contacts_items_upload", uploadKey, 0);
            }
            else if (method == MembersUploadMethod.Auto)
            {
        ...
        public UploadSumarize MembersGroupListAdd(DataTable dtCustomer, int accountId, int groupType, int groupId, int personalFieldsCount, bool cleanBeforInsert)
        {
            ...
            DataTable source = dal.MembersMailingListWithSchema(accountId, groupId);
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    if (cleanBeforInsert)
                    {
                        dal.MembersMailingList_RemoveAll(groupId);
                    }
                    dal.InsertTable(dt, "Members_Mailing_List");
                }
                sum.Ok = count;
        ...
        public UploadSumarize MembersItemsBlockedAdd(DataTable dtCustomer, PlatformType platform, int accountId, int groupId, string userId, string remark, int BlockLevel)
        {
            ...
            DataTable source = dal.MembersItems_BlockedWithSchema(accountId, TargetType);
            DataTable dt = source.Clone();
            ...
            {
                if (TargetType==2)
                    dal.InsertTable(dt, "Members_Items_Blocked_Mail");
                else
                    dal.InsertTable(dt, "Members_Items_Blocked_Cli");

                //dal.InsertTable(dt, "Members_Items_Blocked");
                dal.InsertTable(history, "Members_Items_Blocked_History");
                if (isUpdate)
                {
                    try
                    {
                        if (TargetType == 2)
                            dal.UpdateTable(dt, "Members_Items_Blocked_Mail");
                        else
                            dal.UpdateTable(dt, "Members_Items_Blocked_Cli");

                        //dal.UpdateTable(source, "Members_Items_Blocked");
                    }
        ...
        //public int MemberGroupAdd(ICollection<long> contacts, int accountId, string GroupName)
        //{
        ...
        //    DataTable dtGroups = dal.Members_Groups(accountId,false);
        //    DataView dvGroups = new DataView(dtGroups, "", "MemberGroupName", DataViewRowState.CurrentRows);
        //    int groupId = 0;
        //    int index = dvGroups.Find(GroupName);
        //    if (index > -1)
        //    {
        //        groupId = Types.ToInt(dvGroups[index]["MemberGroupId"], 0);
        //    }
        //    if (groupId <= 0)
        //    {
        //        dal.Members_Groups_Insert(ref groupId, GroupName, accountId);
        //    }
        //    if (groupId <= 0)
        //    {
        ...
        //    return MemberGroupAdd(contacts, accountId, groupId);

        //}

       

        //public void Members_Relation_Remove(int groupId)
        //{
        ...
        public static DataTable Members_Temp_Schema()
        {
            DataTable dt = new DataTable("Members_Temp");
        ...
        //public static void InsertMembersTemp(ICollection<int> contacts, int accountId, int groupId, string key)
        //{
        //    DalMembers.Instance.Members_Temp_Delete(key);

        //    DataTable dt = Members_Temp_Schema();
        //    Guid g = new Guid(key);
        ...
        //    DalMembers.Instance.InsertTable(dt, "Members_Temp");

        //}

        
        //public static Guid InsertMembersTemp(ICollection<int> contacts, int accountId, int groupId)
        //{
        //    Guid g = Guid.NewGuid();
        //    InsertMembersTemp(contacts, accountId, groupId, g.ToString());
        //    return g;
        //}
