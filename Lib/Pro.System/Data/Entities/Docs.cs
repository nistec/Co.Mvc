...
{
    public class DocsContext : EntityModelContext<DocItem>
    {
        const string EntityCacheGroup = EntityCacheGroups.Docs;

        public static void Refresh(int AccountId)
        {
            DbContextCache.Remove<DocItem>(Settings.ProjectName, EntityCacheGroup, AccountId, 0);
        }
        public static DocsContext Get(int AccountId)
        {
            return new DocsContext(AccountId);
        }
