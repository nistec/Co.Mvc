...
namespace Netcell.Remoting
{
    ...
    public static class RemoteUtil
    {
        ...
        public static string FormatMessage(string message, string personal)
        {
            ...
            {
                throw new NetException(AckStatus.ArgumentException, "Message not match to personal attributes");
            }
            return str2;
        }
        ...
        public static string BuildMessage(string message, string personal, string[] args)
        {
            ...
            {
                throw new NetException(AckStatus.ArgumentException, "Message not match to personal attributes args");
            }
            return str2;
        }
...
/*
        public static IQueueItem CreateRemoteCellItem(Priority priority, int msgId, int transId, int accountId, string method, string sender, string message, string url, string target, int campaignId)
        {
            ...
            decimal price = 0;
            BillingItem.ValidateCredit(accountId, methodType, 1, UnitsItem.CreateUnitsItem(methodType, message, true), ref price);
            IQueueItem item = CreateMessageItem(priority, message,transId, campaignId, msgId, accountId, methodType, billType, true, sender, cli.CellNumber, price);
            return item;
        ...
        public static IQueueItem CreateMessageItem(
                Priority priority,
                string message,
                int transId,
                int campaignId,
                ...
                decimal price)
        {
            ...
            try
            {
                ...
                if (billingType == BillingType.LB)
                    operatorId = SrvConfig.LabOperator;
                else
                {
                    CLI cli = new CLI(target);
                    if (!cli.IsValid)
        ...
        public static IQueueItem CreateRemoteMailItem(Priority priority,int msgId, int transId,int accountId, string from, string display, string replyTo, string target, string subject, string body, int campaignId)
        {
            MethodType methodType = MethodType.MALMT;
            int itemsCount = 1;
            BillingItem bi = BillingItem.CreateBillingItem(accountId, methodType, new UnitsItem(methodType, body, true), itemsCount, 0, true);
            //mailhost
            IMailHost mcnn = MailHost.CreateQuickConnection(accountId);
            IQueueItem item = CreateMailQueueItem(priority,msgId, campaignId, transId, accountId, from, display, replyTo, target, subject, body, bi.ItemPrice, bi.ItemUnits);

            return item;
        }

        public static IQueueItem CreateMailQueueItem(
       Priority priority,
       int msgId,
       int campaignId,
       int batchId,
