...
namespace Netcell.Remoting
{
    ...
    public class BillingItem
    {
        ...
        */
        ...
        public static CreditState CheckCredit(int AccountId, MethodType Method, int Count, UnitsItem ui)
        {
            ...
            //if (units == 0)
            //{
            //    throw new NetException(AckStatus.ArgumentException, "Units must be greater then zero");
            //}
            int totalUnits = Count * units;

            int res = DalRule.ValidateCredit(AccountId, (int)Method, totalUnits);
        ...
        public static bool ValidateCredit(int AccountId, MethodType Method, int Count, int Units)
        {
            if (Count == 0)
            {
                throw new NetException(AckStatus.BadDestination, "Invalid destination count");
            }
            if (Units == 0)
            {
                throw new NetException(AckStatus.ArgumentException, "Units must be greater then zero");
            ...
            switch ((CreditState)res)
            {
                case CreditState.InvalidBillingPrice://-1
                    throw new NetException(AckStatus.BillingException, "Invalid Billing Price");
                case CreditState.NotEnoughCredit://0
                    throw new NetException(AckStatus.NotEnoughCredit, string.Format("Not Enough Credit, {0}", AccountId));
                case CreditState.OkAlertFlag://3
                    //ActiveSystemAlert.SendCreditAlert(AccountId, ViewConfig.CreditMessage);
                    RemoteSystemAlerts.ExecuteSystemAlert(1, (int)AckStatus.WarningAlert, AccountId, ViewConfig.CreditMessage, 0, "BillingItem");
                    return true;
                default:
                    return true;
            }
        ...
        public static bool ValidateCredit(int AccountId, MethodType Method, int Count, int ItemUnits, ref decimal Price)
        {
            ...
            switch ((CreditState)res)
            {
                case CreditState.InvalidItems://-1
                    throw new NetException(AckStatus.BadDestination, "Invalid destination count or Items units");
                case CreditState.InvalidBillingPrice://-1
                    throw new NetException(AckStatus.BillingException, "Invalid Billing Price");
                case CreditState.NotEnoughCredit://0
                    throw new NetException(AckStatus.NotEnoughCredit, string.Format("Not Enough Credit, {0}", AccountId));
        ...
        public static int GetBillingUnits(int OperatorId, MethodType Method, UnitsItem ui)
        {
            ...
            if (MsgMethod.IsSms(Method, true) && ui.Concat)
            {
                if (ui.Length <= 0)
                {
                    throw new NetException(AckStatus.InvalidContent, "Invalid message");
                }
    ...
    public class BillingRB
    {
        ...
        public static int ValidtePricingRB(string priceCode, string SC)
        {
            ...
            {
                throw new NetException(AckStatus.InvalidServiceCode, "Invalid price code");
            }
    ...
    public class UnitsItem
    {
        ...
        public int GetSMSBillingUnits(byte bunch, int OperatorId = 0)
        {
            ...
            if (this.Length <= 0)
            {
                throw new NetException(AckStatus.InvalidContent, "Invalid message length");
            }
