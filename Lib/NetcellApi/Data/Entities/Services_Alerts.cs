    ...
    public class Services_Alerts_Context : EntityContext<Services_Alerts>
    {
       ...
       public Services_Alerts_Context(int AlertId)
        ...
        public static int ServiceAlert_Completed(int AlertId, int State)
        {
            using (var cmd = Netcell_Services.Instance)
            {
                return cmd.ExecuteCommandNonQuery("sp_ServiceAlert_Completed", DataParameter.Get("AlertId",AlertId,"State", State), CommandType.StoredProcedure);
            }
        }
        ...
        public static DataTable GetList(int ItemId)
        {
            using (var cmd = Netcell_Services.Instance)
            {
                return cmd.ExecuteCommand<DataTable>("select * from Services_Alerts where ItemId=@ItemId", DataParameter.Get("ItemId", ItemId));
            }
        }

        public static Services_Alerts Deserialize(string base64)
        {
            return NetSerializer.DeserializeFromBase64<Services_Alerts>(base64);
        }
