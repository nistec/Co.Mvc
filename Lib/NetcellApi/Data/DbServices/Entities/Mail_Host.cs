    ...
    public class Mail_Host_Context : EntityContext<Mail_Host>
    {
       ...
       public Mail_Host_Context(DataFilter filter)
        ...
        public static Mail_Host Get(int AccountId)
        {
            ...
            {
                dr = cmd.ExecuteCommand<DataRow>("SELECT top 1 * from [vw_Mailer_Host] where (AccountId=@AccountId or AccountId=0) and IsActive=1 order by AccountId desc", DataParameter.Get("AccountId", AccountId), CommandType.Text);
             }
            if (dr == null)
            {
                return null;
        ...
        {
            return NetSerializer.DeserializeFromBase64<Mail_Host>(base64);
        }
