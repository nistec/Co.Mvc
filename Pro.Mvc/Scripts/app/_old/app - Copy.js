...
function accountDisplay(val, title, divResult) {
    $.ajax({
        type: "POST",
        url: getAppUrl("/Ws/CrmWs.asmx/GetAccountDetails"),
        data: "{ 'id': '" + val + "' }",
...
function accountEdit(id, acctype) {
    return popupIframe(getAppUrl("/Common/_AccountEdit?id=") + id + "&acctype=" + acctype, "500", "580");
};

function accountNewsAdd(id) {
    return popupIframe(getAppUrl("/Common/_AccountNewsEdit?id=") + id, "400", "550");
};
...
function accountNewsRemove(newsid, accid) {
    if (confirm("האם להסיר לקוח " + accid + " מקבוצת דיוור " + newsid)) {
        $.ajax({
            type: "POST",
            url: getAppUrl("/Ws/CrmWs.asmx/AccountNewsDelete"),
...
function accountContacts(id) {
    popupIframe(getAppUrl("/Common/_AccountContacts?id=") + id , "600", "320");
};
function contactDisplay(val, title, divResult) {
    $.ajax({
        type: "POST",
        url: getAppUrl("/Ws/CrmWs.asmx/GetContactDetails"),//"/Ws/CrmWs.asmx/GetContactDetails",
        data: "{ 'id': '" + val + "' }",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
...
function contactEdit(id, accid, op) {
    if (op == "d")
        return dialogIframe(getAppUrl("/Common/_ContactEdit?id=") + id + "&accid=" + accid + "&op=" + op, "300", "250", "אנשי קשר");
    else
        return popupIframe(getAppUrl("/Common/_ContactEdit?id=") + id + "&accid=" + accid + "&op=p", "320", "320");
};
...
function contactDelete(id, async) {
    ...
    if (confirm("האם למחוק איש קשר " + id)) {
        $.ajax({
            async: async,
            type: "POST",
            url: getAppUrl("/Ws/CrmWs.asmx/ContactDelete"),
            data: "{ 'id': '" + id + "' }",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data) {
...
function mediaEditor(bid, pid, pt) {
    popupIframe(getAppUrl("/Building/_Media?bid=") + bid + "&pid=" + pid + "&pt=" + pt, "900", "520");
};
...
var createListAdapter = function (valueMember, displayMember, tag, url, width, height, ismulti) {
    var srcAdapter = createtDataAdapter(valueMember, displayMember, url);
    $("#" + tag).jqxListBox(
    {
        rtl: true,
    ...
    if (ismulti)
        $("#" + tag).jqxListBox({multiple: true});
    return srcAdapter;
};
var createtDataAdapter = function (valueMember, displayMember, url, data) {
    var source =
            {
                dataType: "json",
                dataFields: [
