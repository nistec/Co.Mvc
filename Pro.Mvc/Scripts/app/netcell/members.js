...
/*
        var initSginupGrid = function (tab, index, id) {
            ...
            var nastedsource = {
                ...
                //id: 'SignupId',
                type: 'POST',
                url: '/Netcell/GetMemberSignupHistory',
                data: { 'rcdid': id }
            }
...
*/
        var initCategoriesGrid = function (tab, index, id) {
            ...
            var nastedsource = {
                ...
                id: 'PropId',
                type: 'POST',
                url: '/Netcell/GetMemberCategories',
                data: { 'rcdid': id }
            }
    ...
    categoriesDelete(id, rcdid) {
        ...
        if (confirm("האם להסיר מנוי " + rcdid + " מקבוצת סיווג " + id)) {
            $.ajax({
                type: "POST",
                url: '/Netcell/DeleteMemberCategories',
    ...
    query() {
        //app.redirectTo('/Netcell/MembersQuery');
        if (!wizard.existsIframe(3)) {
            wizard.appendIframe(3, app.appPath() + "/Netcell/_MembersQuery", "100%", "680px", true, "#loader");
        }
    ...
    add() {
        ...
        //wizard.appendIframe(2, app.appPath() + "/Netcell/_MemberAdd", "100%", "620px", true, "#loader");
    ...
    edit(id) {
        ...
        if (id > 0) {
            ...
            //wizard.appendIframe(2, app.appPath() + "/Netcell/_MemberEdit?id=" + id, "100%", "620px", true, "#loader");
        }
    }
    ...
    remove(id) {
        ...
        if (id > 0) {
            if (confirm('האם למחוק את המנוי ' + id)) {
                app_query.doPost(app.appPath() + "/Netcell/MemberDelete", { 'RecordId': id }, this.end_internal);
            }
        }
    }
    ...
    end_plus(data) {
        ...
        app_messenger.Post(data);
        if (data && data.Status > 0) {
            wizard.changeIframe(app.appPath() + "/Netcell/_MemberAdd");
        }
    }
        ...
        this.dataSource =
            {
                ...
                type: 'POST',
                url: '/Netcell/GetMemberEdit'
            };
        var pasive = dataModel.Option == "a" ? " pasive" : "";
        var html =
            '<div id="fcWindow">' +
            '<div id="fcBody">' +
            '<form class="fcForm" id="fcForm" method="post" action="/Netcell/MemberUpdate">' +
    ...
    init() {
        ...
        $('#submitScreen').on('click', function () {
            ...
            if (_slf.IsDialog) {
                ...
                var action = '/Netcell/Members';
                $("#form").attr('action', app.appPath() + action);
                $("#form").submit();
            }
        });
        $('#submitExport').on('click', function () {
            var action = '/Netcell/MembersExport';
            $("#QueryType").val(20);
            $("#form").attr('action', app.appPath() + action);
            $("#form").submit();
        });
