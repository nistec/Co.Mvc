...
Vromansys.Item.Calculation = function(el) {
	var _el = $(el);
	var _form = $(Vromansys.Form.getForm());
	var _button = _el.find("table.calculation button");
	var _span = _el.find("table.calculation span");
...
Vromansys.Item.TextList = function(el) {
	...
	function _addRow(row) {
		...
		// setup events
		row.find("td img:first").on("click", Vromansys.Util.createCallback(_addRow, [ row ]));
		row.find("td img:last").on("click", Vromansys.Util.createCallback(_removeRow, [ row ]));
	...
	return {
		init: function() {
			...
			rows.each(function(n, r) {
				...
				r.find("td img:first").on("click", Vromansys.Util.createCallback(_addRow, [ r ]));
				r.find("td img:last").on("click", Vromansys.Util.createCallback(_removeRow, [ r ]));
